    ...
    public function deleteCustomer(int $customer_id): bool
    {
        try {
            ...
            $stmt = $this->conn->prepare(
                "DELETE FROM customer_offers WHERE customer_id = :customer_id"
            );
            $stmt->execute(['customer_id' => $customer_id]);

            $stmt = $this->conn->prepare(
                "SELECT COUNT(*) FROM customer_offers WHERE customer_id = :customer_id"
            );
            $stmt->execute(['customer_id' => $customer_id]);

            if ($stmt->fetchColumn() > 0) {
                throw new Exception("Customer still has related offers");
            }
